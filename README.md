# CData Dremio ARP Connector for RSS Feeds

The RSS Feeds connector allows Dremio to connect to and query data in RSS Feeds. This can then allow you to build custom reports, dashboards, or even just ad-hoc SQL via your client tool of choice. Note that it does require a third-party JDBC driver that is not free, but does allow a free trial.
...
3. Download and install the [RSS Feeds JDBC Driver from CData](https://www.cdata.com/drivers/rss/download/jdbc)* and copy the JAR file to the /jars/3rdparty/ directory of your Dremio installation. For example:
